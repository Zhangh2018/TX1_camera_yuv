...
int Camera::read_frame(unsigned char *image) {
    ...
    memcpy(image,buffers[buf.index].start,cap_image_size);

    printf("cap_image_size = %d\n", cap_image_size);    
...
bool Camera::init_device(void) {
	...
	printf("xioctl(fd, VIDIOC_QUERYCAP, &cap) error\n");	
        if (EINVAL == errno) {
    ...
    if(c_camera_type == 0)
    {
    	...
    	{	
		printf("-1 == xioctl(fd, VIDIOC_S_FMT, &fmt)\n");
        	return false;
     	}
     }
